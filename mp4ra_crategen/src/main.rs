...
// --------");
    ...
    generic::GenericGenerator::new("BrandCode")
        .gen(&database.load::<Record>("brands.csv").expect("Failure generating brands entries"), &mut scope);

    generic::GenericGenerator::new("TrackReferenceCode")
        .gen(&database.load::<Record>("track-references.csv").expect("Failure generating track-reference entries"), &mut scope);
