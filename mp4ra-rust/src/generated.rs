...
impl BoxCode {
    /// Return a human-readable description for this box code, if one is known.
    pub fn description(&self) -> Option<&'static str> {
        match *self {
            BoxCode::COMPRESSED_MOF => Some("Compressed movie fragment"),
            BoxCode::COMPRESSED_MOV => Some("Compressed movie"),
            BoxCode::COMPRESSED_SIX => Some("Compressed segment index"),
            BoxCode::COMPRESSED_SSX => Some("Compressed subsegment index"),
            BoxCode::TWO_DQR => Some("2D region quality ranking"),
            BoxCode::TWO_DSS => Some("spatial relationship 2D source"),
            BoxCode::THREE_DPR => Some("MVDDepthResolutionBox"),
            BoxCode::THREE_SIB => Some("MVD Scalability Information SEI Message Box"),
            BoxCode::SIX_VPC => Some("Viewport Configuration"),
            BoxCode::EXIF => Some("Exif Metadata"),
            BoxCode::ID32 => Some("ID3 version 2 container"),
            BoxCode::JXL => Some("JPEG XL Signature"),
            BoxCode::JXS => Some("JPEG XS Signature"),
            BoxCode::SMDM => Some("Mastering Display Metadata Box"),
            BoxCode::UITS => Some("Unique Identifier Technology Solution"),
            BoxCode::A3DC => Some("A3DConfigurationBox"),
            BoxCode::ACXD => Some("Decoder-specific info for Auro-Cx audio"),
            BoxCode::ADES => {
                Some(
                    "description of the content for accessibility purposes (similar to the HTML alt attribute); formatted as for the cprt user-data item",
                )
            }
            BoxCode::AEDB => Some("audio element description"),
            BoxCode::AELM => Some("audio element box"),
            BoxCode::AEPP => Some("audio element positioning interactivity polar box"),
            BoxCode::AEPR => Some("audio element prominence interactivity box"),
            BoxCode::AESB => Some("audio element selection box"),
            BoxCode::AESD => Some("audio element selection description box"),
            BoxCode::AINF => Some("Asset information to identify, license and play"),
            BoxCode::ALBM => Some("Album title and track number for media"),
            BoxCode::ALMO => {
                Some("Alpha mode (straight vs premultiplied alpha) signaling")
            }
            BoxCode::ALOU => Some("Album loudness base"),
            BoxCode::AMVE => Some("Ambient viewing environment"),
            BoxCode::ANGL => {
                Some("Name of the camera angle through which the clip was shot")
            }
            BoxCode::APVC => Some("APV Codec Configuration Box"),
            ...
            BoxCode::BIDX => Some("Box Index"),
            BoxCode::BLOC => {
                Some("Base location and purchase location for license acquisition")
            }
            BoxCode::BMDM => Some("Buffer Model Description"),
            BoxCode::BPCC => Some("Bits per component"),
            BoxCode::BROB => Some("Brotli-compressed box"),
            ...
            BoxCode::CDEF => {
                Some("type and ordering of the components within the codestream")
            }
            BoxCode::CHNL => Some("Channel layout"),
            BoxCode::CINF => Some("complete track information"),
            BoxCode::CLAP => Some("Clean aperture"),
            BoxCode::CLEV => Some("Component Reference Level"),
            BoxCode::CLFN => Some("Name of the clip file"),
            BoxCode::CLID => Some("Identifier of the clip"),
            BoxCode::CLIP => Some("Visual clipping region container"),
            BoxCode::CLLI => Some("Content Light Level"),
            BoxCode::CLOC => Some("Chroma Location"),
            BoxCode::CLSF => Some("Classification of the media"),
            BoxCode::CMAP => Some("mapping between a palette and codestream components"),
            BoxCode::CMID => Some("Identifier of the camera"),
            BoxCode::CMNM => Some("Name that identifies the camera"),
            BoxCode::CMPD => Some("Component Definition"),
            BoxCode::CO64 => Some("64-bit chunk offset"),
            BoxCode::COIF => Some("composition information"),
            BoxCode::COIN => Some("Content Information Box"),
            BoxCode::COLL => Some("Name of the collection from which the media comes"),
            BoxCode::COLR => Some("specifies the colourspace of the image"),
            BoxCode::COVI => Some("coverage information"),
            BoxCode::CPAL => Some("Component Palette Configuration"),
            BoxCode::CPAT => Some("Component Pattern Definition"),
            BoxCode::CPRT => Some("copyright etc."),
            BoxCode::CRGN => Some("Visual clipping region definition"),
            BoxCode::CRHD => Some("reserved for ClockReferenceStream header"),
            BoxCode::CSGM => {
                Some("Compact sample to group used in QuickTime File Format")
            }
            BoxCode::CSGP => Some("compact sample to group"),
            ...
            BoxCode::DAC3 => Some("Decoder specific info for AC-3 audio"),
            BoxCode::DAC4 => Some("Decoder specific info for AC-4 audio"),
            BoxCode::DATE => {
                Some(
                    "Date and time, formatted according to ISO 8601, when the content was created. For clips captured by recording devices, this is typically the date and time when the clip’s recording started.",
                )
            }
            BoxCode::DCA3 => Some("Decoder configuration for AVS3-P3 Codec"),
            BoxCode::DCFD => Some("Marlin DCF Duration, user-data atom type"),
            ...
            BoxCode::DISI => Some("Disparity Information"),
            BoxCode::DMIX => Some("Downmix instructions"),
            BoxCode::DMLP => Some("Decoder specific info for MLP audio"),
            BoxCode::DMON => Some("Mastering Display Metadata"),
            BoxCode::DREF => {
                Some("data reference box, declares source(s) of media data in track")
            }
            BoxCode::DSCP => Some("Media description"),
            BoxCode::DSGD => Some("DVB Sample Group Description Box"),
            BoxCode::DSTG => Some("DVB Sample to Group Box"),
            BoxCode::DVCC => Some("Dolby Vision Configuration"),
            BoxCode::DVVC => Some("Dolby Vision Extended Configuration"),
            BoxCode::DVWC => Some("Dolby Vision Extended Configuration 2"),
            BoxCode::ECAM => Some("Extrinsic camera parameters"),
            BoxCode::EDTS => Some("edit list container"),
            BoxCode::ELNG => Some("Extended Language Tag"),
            BoxCode::ELST => Some("an edit list"),
            BoxCode::EMSG => Some("event message"),
            BoxCode::ESDS => Some("Elementary stream descriptor"),
            BoxCode::ETYP => Some("extended type and type combination"),
            BoxCode::EVCC => Some("EVC configuration"),
            BoxCode::EVSC => Some("Configuration for EVC slice component track"),
            BoxCode::EVTI => Some("Event information"),
            BoxCode::FDEL => Some("File delivery information (item info extension)"),
            BoxCode::FECI => Some("FEC Informatiom"),
            BoxCode::FECR => Some("FEC Reservoir"),
            BoxCode::FIDX => Some("Box File Index"),
            BoxCode::FIEL => Some("Field Coding"),
            BoxCode::FIIN => Some("FD Item Information"),
            BoxCode::FIRE => Some("File Reservoir"),
            BoxCode::FOVD => Some("fisheye omnidirectional video"),
            BoxCode::FOVI => Some("fisheye video essential info"),
            BoxCode::FPAC => Some("Frame Packing Information"),
            BoxCode::FPAR => Some("File Partition"),
            BoxCode::FREE => Some("free space"),
            BoxCode::FRMA => Some("original format box"),
            BoxCode::FRPA => Some("Front Part"),
            BoxCode::FTYP => Some("file type and compatibility"),
            BoxCode::FVSI => Some("fisheye video supplemental info"),
            BoxCode::GITN => Some("Group ID to name"),
            BoxCode::GNRE => Some("Media genre"),
            BoxCode::GRPI => Some("OMA DRM Group ID"),
            BoxCode::GRPL => Some("Groups list box"),
            BoxCode::HDLR => Some("handler, declares the media (handler) type"),
            BoxCode::HINF => Some("hint information"),
            BoxCode::HMHD => {
                Some("hint media header, overall information (hint track only)")
            }
            BoxCode::HNTI => Some("Hint information"),
            BoxCode::HPIX => Some("Hipix Rich Picture (user-data or meta-data)"),
            BoxCode::HVCC => Some("HEVC Configuration"),
            BoxCode::HVTC => Some("HEVC Tile Configuration"),
            BoxCode::ICAM => Some("Intrinsic camera parameters"),
            BoxCode::ICNU => Some("OMA DRM Icon URI"),
            BoxCode::IDAT => Some("Item data"),
            BoxCode::IHDR => Some("Image Header"),
            BoxCode::IINF => Some("item information"),
            BoxCode::ILOC => Some("item location"),
            BoxCode::IMAP => Some("Track input map definition"),
            BoxCode::IMDA => Some("Identified media data"),
            BoxCode::IMIF => Some("IPMP Information box"),
            BoxCode::INFE => Some("Item information entry"),
            BoxCode::INFU => Some("OMA DRM Info URL"),
            BoxCode::IODS => Some("Object Descriptor container box"),
            BoxCode::IPCO => Some("ItemPropertyContainerBox"),
            BoxCode::IPHD => Some("reserved for IPMP Stream header"),
            BoxCode::IPMA => Some("ItemPropertyAssociation"),
            BoxCode::IPMC => Some("IPMP Control Box"),
            BoxCode::IPRO => Some("item protection"),
            BoxCode::IPRP => Some("Item Properties Box"),
            BoxCode::IREF => Some("Item reference"),
            BoxCode::IROI => Some("SVC region of interest box"),
            ...
            BoxCode::JP2I => Some("intellectual property information"),
            BoxCode::JPGC => Some("JPEG Configuration"),
            BoxCode::JPTP => Some("JPEG XS Video Transport Parameter"),
            BoxCode::JPVI => Some("JPEG XS Video Information"),
            BoxCode::JPVS => Some("JPEG XS Video Support"),
            BoxCode::JUMB => Some("JPEG Universal Metadata Box Format (JUMBF)"),
            BoxCode::JXLC => Some("JPEG XL Codestream"),
            BoxCode::JXLI => Some("JPEG XL Frame Index"),
            BoxCode::JXLL => Some("JPEG XL Level"),
            BoxCode::JXLP => Some("JPEG XL Partial Codestream"),
            BoxCode::JXPL => Some("JPEG XS Profile and Level"),
            BoxCode::KIND => Some("Track kind"),
            BoxCode::KMAT => Some("Compressed visual track matte"),
            BoxCode::KYWD => Some("Media keywords"),
            BoxCode::LABL => Some("Label box"),
            BoxCode::LDEP => Some("Tier dependency box"),
            BoxCode::LEQI => Some("Loudness equalization instructions"),
            BoxCode::LEVA => Some("Leval assignment"),
            BoxCode::LHVC => Some("Layered HEVC Configuration"),
            BoxCode::LOAD => Some("Track pre-load definitions"),
            BoxCode::LOCI => Some("Media location information"),
            BoxCode::LOOP => Some("Looping behavior"),
            BoxCode::LRCU => Some("OMA DRM Lyrics URI"),
            BoxCode::LUDT => Some("Track loudness container"),
            BoxCode::LVCC => Some("LCEVC configuration"),
            BoxCode::M4DS => Some("MPEG-4 descriptors"),
            BoxCode::M7HD => Some("reserved for MPEG7Stream header"),
            BoxCode::MAEI => Some("MPEG-H Audio scene information"),
            BoxCode::MAEM => Some("MPEG-H Audio multi-stream signalling"),
            BoxCode::MANU => Some("Manufacturer name of the camera"),
            BoxCode::MATT => Some("Visual track matte for compositing"),
            BoxCode::MD5I => Some("MD5IntegrityBox"),
            BoxCode::MDAT => Some("media data container"),
            BoxCode::MDCV => Some("mastering display colour volume"),
            BoxCode::MDHD => Some("media header, overall information about the media"),
            BoxCode::MDIA => Some("container for the media information in a track"),
            BoxCode::MDRI => Some("Mutable DRM information"),
            BoxCode::MECO => Some("additional metadata container"),
            BoxCode::MEHD => Some("movie extends header box"),
            BoxCode::MEOF => Some("media offset"),
            BoxCode::MERE => Some("metabox relation"),
            BoxCode::MESH => Some("mesh box"),
            BoxCode::META => Some("Metadata container"),
            BoxCode::MFHD => Some("movie fragment header"),
            BoxCode::MFRA => Some("Movie fragment random access"),
            BoxCode::MFRO => Some("Movie fragment random access offset"),
            BoxCode::MHAC => Some("MPEG-H Audio Decoder Configuration"),
            BoxCode::MHAD => Some("MPEG-H Audio dynamic range control and loudness"),
            BoxCode::MHAP => Some("MPEG-H Audio profile and level compatibility set"),
            BoxCode::MINF => Some("media information container"),
            BoxCode::MINI => {
                Some(
                    "The MinimizedImageBox provides a more compact representation of the MetaBox for a subset of use cases",
                )
            }
            BoxCode::MJHD => Some("reserved for MPEG-J Stream header"),
            BoxCode::MMVI => Some("multimap video"),
            BoxCode::MODL => Some("Model name of the camera"),
            BoxCode::MOOF => Some("movie fragment"),
            BoxCode::MOOV => Some("container for all the meta-data"),
            BoxCode::MOVD => Some("mesh omnidirectional video"),
            BoxCode::MSTV => Some("MVC sub track view box"),
            BoxCode::MVCC => Some("MVC configuration"),
            BoxCode::MVCP => Some("MVC priority assignment"),
            BoxCode::MVCG => Some("Multiview group"),
            BoxCode::MVCI => Some("Multiview Information"),
            BoxCode::MVDC => Some("MVCDConfigurationBox"),
            BoxCode::MVEX => Some("movie extends box"),
            BoxCode::MVHD => Some("movie header, overall declarations"),
            BoxCode::MVRA => Some("Multiview Relation Attribute"),
            BoxCode::NMHD => {
                Some("Null media header, overall information (some tracks only)")
            }
            BoxCode::OCHD => Some("reserved for ObjectContentInfoStream header"),
            BoxCode::ODAF => Some("OMA DRM Access Unit Format"),
            BoxCode::ODDA => Some("OMA DRM Content Object"),
            BoxCode::ODHD => Some("reserved for ObjectDescriptorStream header"),
            BoxCode::ODHE => Some("OMA DRM Discrete Media Headers"),
            BoxCode::ODRB => Some("OMA DRM Rights Object"),
            BoxCode::ODRM => Some("OMA DRM Container"),
            BoxCode::ODTT => Some("OMA DRM Transaction Tracking"),
            BoxCode::OHDR => Some("OMA DRM Common headers"),
            BoxCode::ORIE => Some("Orientation information"),
            BoxCode::OTCF => Some("OMAF timed text configuration"),
            BoxCode::OTYP => Some("Original file type"),
            BoxCode::OVLY => Some("overlay configuration"),
            BoxCode::PADB => Some("sample padding bits"),
            BoxCode::PAEN => Some("Partition Entry"),
            BoxCode::PASP => Some("Pixel aspect ratio"),
            BoxCode::PCLR => {
                Some(
                    "palette which maps a single component in index space to a multiple- component image",
                )
            }
            BoxCode::PCMC => Some("Uncompressed audio PCM configuration"),
            BoxCode::PDAT => Some("Partial Data"),
            BoxCode::PDC1 => Some("Parametric DRC coefficients"),
            BoxCode::PDI1 => Some("Parametric DRC instructions"),
            BoxCode::PDIN => Some("Progressive download information"),
            BoxCode::PERF => Some("Media performer name"),
            BoxCode::PFHD => Some("Partial File Header"),
            BoxCode::PFIL => Some("Partial File"),
            BoxCode::PITM => Some("primary item reference"),
            BoxCode::PLOC => Some("Partial Segment Location"),
            BoxCode::PNOT => Some("Preview container"),
            BoxCode::POVD => Some("projected omnidirectional video"),
            BoxCode::PRFR => Some("projection format"),
            BoxCode::PRFT => Some("Producer reference time"),
            BoxCode::PRMD => Some("Production metadata box"),
            BoxCode::PRMR => Some("Production metadata reference box"),
            BoxCode::PSEG => Some("Partial Segment"),
            BoxCode::PSHD => Some("Partial Segment Header"),
            BoxCode::PSSH => Some("Protection system specific header"),
            BoxCode::PTLE => Some("Partial Top Level Entry"),
            BoxCode::QLIF => Some("Layer quality assignments"),
            BoxCode::REEL => Some("Name of the tape reel"),
            BoxCode::RES => Some("grid resolution"),
            BoxCode::RESC => Some("grid resolution at which the image was captured"),
            BoxCode::RESD => {
                Some("default grid resolution at which the image should be displayed")
            }
            BoxCode::RINF => Some("restricted scheme information box"),
            BoxCode::ROSC => Some("sphere region configuration"),
            BoxCode::ROTN => Some("rotation box"),
            BoxCode::RRGN => Some("SVC rect region box"),
            BoxCode::RSOT => Some("Redundant Sample Original Timing"),
            BoxCode::RTNG => Some("Media rating"),
            BoxCode::RVIF => Some("recommended viewport information"),
            ...
            BoxCode::SCHI => Some("scheme information box"),
            BoxCode::SCHM => Some("scheme type box"),
            BoxCode::SCRB => Some("Scramble scheme information"),
            BoxCode::SDEP => Some("Sample dependency"),
            BoxCode::SDHD => Some("reserved for SceneDescriptionStream header"),
            BoxCode::SDP => Some("SDP information"),
            BoxCode::SDTP => Some("Independent and Disposable Samples Box"),
            BoxCode::SDVP => Some("SD Profile Box"),
            BoxCode::SEGR => Some("file delivery session group"),
            BoxCode::SEIB => Some("Scalability information"),
            BoxCode::SEII => Some("SEI information box"),
            BoxCode::SENC => Some("Sample specific encryption data"),
            BoxCode::SGPD => Some("Sample group definition box"),
            BoxCode::SHOT => Some("Name that identifies the shot"),
            BoxCode::SIDX => Some("Segment Index Box"),
            BoxCode::SILB => Some("Scheme ID list Box"),
            BoxCode::SINF => Some("protection scheme information box"),
            BoxCode::SKIP => Some("free space"),
            BoxCode::SLNO => Some("Serial number of the camera"),
            BoxCode::SMHD => {
                Some("sound media header, overall information (sound track only)")
            }
            BoxCode::SNUC => Some("Sensor Non-Uniformity Correction"),
            BoxCode::SPKI => Some("Sample packing information box"),
            BoxCode::SPLZ => Some("Polarization Pattern Definition"),
            BoxCode::SPRG => Some("sub-picture region"),
            BoxCode::SRAT => Some("Sampling rate"),
            BoxCode::SRMB => Some("System Renewability Message"),
            BoxCode::SRMC => Some("System Renewability Message container"),
            BoxCode::SRPP => Some("STRP Process"),
            BoxCode::SRQR => Some("sphere region quality ranking"),
            BoxCode::SSIX => Some("Sub-sample index"),
            BoxCode::SSRT => Some("Sub-sample Reference Table Box"),
            BoxCode::SSTL => Some("SVC sub track layer box"),
            BoxCode::STBL => Some("sample table box, container for the time/space map"),
            BoxCode::STCO => Some("chunk offset, partial data-offset information"),
            BoxCode::STDP => Some("sample degradation priority"),
            BoxCode::STHD => Some("Subtitle Media Header Box"),
            BoxCode::STMG => Some("MVC sub track multiview group box"),
            BoxCode::STRD => Some("Sub-track definition"),
            BoxCode::STRI => Some("Sub-track information"),
            BoxCode::STRK => Some("Sub track information"),
            BoxCode::STSC => Some("sample-to-chunk, partial data-offset information"),
            BoxCode::STSD => {
                Some("sample descriptions (codec types, initialization etc.)")
            }
            BoxCode::STSG => Some("Sub-track sample grouping"),
            BoxCode::STSH => Some("shadow sync sample table"),
            BoxCode::STSS => Some("sync sample table (random access points)"),
            BoxCode::STSZ => Some("sample sizes (framing)"),
            BoxCode::STTI => Some("Sub track tier box"),
            BoxCode::STTS => Some("(decoding) time-to-sample"),
            BoxCode::STVI => Some("Stereo Video Box"),
            BoxCode::STYP => Some("Segment Type Box"),
            BoxCode::STZ2 => Some("compact sample sizes (framing)"),
            BoxCode::SUBS => Some("Sub-sample information"),
            BoxCode::SUEP => Some("signer identity information"),
            BoxCode::SUMI => Some("supplemental surveillance meta information"),
            BoxCode::SURL => Some("Source URL"),
            BoxCode::SVCC => Some("SVC configuration"),
            BoxCode::SVCP => Some("SVC priority assignments"),
            BoxCode::SVDR => Some("SVC dependency range"),
            BoxCode::SVIP => Some("Initial parameter sets box for tiers"),
            BoxCode::SVMC => Some("SVC information configuration"),
            BoxCode::SVPR => Some("Priority range"),
            BoxCode::SWRE => {
            ...
            BoxCode::TFHD => Some("Track fragment header"),
            BoxCode::TFMA => Some("Track fragment media adjustment box"),
            BoxCode::TFRA => Some("Track fragment radom access"),
            BoxCode::THMB => Some("Thumbnail image of the media"),
            BoxCode::TIBR => Some("Tier Bit rate"),
            BoxCode::TIRI => Some("Tier Information"),
            BoxCode::TITL => Some("Media title"),
            BoxCode::TKHD => Some("Track header, overall information about the track"),
            BoxCode::TLOU => Some("Track loudness base"),
            BoxCode::TRAF => Some("Track fragment"),
            BoxCode::TRAK => Some("container for an individual track or stream"),
            BoxCode::TRAN => Some("SVC lightweight transcoding"),
            BoxCode::TREF => Some("track reference container"),
            BoxCode::TREP => Some("track extension properties"),
            BoxCode::TREX => Some("track extends defaults"),
            BoxCode::TRGR => Some("Track grouping information"),
            BoxCode::TRIK => Some("Facilitates random access and trick play modes"),
            BoxCode::TRUN => Some("track fragment run"),
            BoxCode::TSEL => Some("Track selection"),
            BoxCode::TSTB => Some("TileSubTrackGroupBox"),
            BoxCode::TTYP => Some("track type and compatibility"),
            BoxCode::TXTC => Some("Text stream configuration"),
            BoxCode::TYCO => Some("type and-combination"),
            BoxCode::UDC1 => Some("Basic DRC coefficients"),
            BoxCode::UDC2 => Some("Unified DRC coefficients"),
            BoxCode::UDEX => Some("Unified DRC configuration extension"),
            BoxCode::UDI1 => Some("Basic DRC instructions"),
            BoxCode::UDI2 => Some("Unified DRC instructions"),
            BoxCode::UDTA => Some("user-data"),
            BoxCode::UEQC => Some("Equalization coefficients"),
            BoxCode::UEQI => Some("Equalization instructions"),
            BoxCode::UINF => {
                Some(
                    "a tool by which a vendor may provide access to additional information associated with a UUID",
                )
            }
            BoxCode::ULST => Some("a list of UUID’s"),
            BoxCode::UNCC => Some("Uncompressed Frame Configuration"),
            BoxCode::URAT => Some("User 'star' rating of the media"),
            BoxCode::URII => Some("URI Information"),
            BoxCode::URL => Some("a URL"),
            BoxCode::UUID => Some("user-extension box"),
            BoxCode::V3CC => Some("V3C Configuration"),
            BoxCode::V3SC => Some("V3C spatial region collection"),
            BoxCode::V3TC => Some("V3C Atlas Tile Configuration"),
            BoxCode::VEXU => Some("Video Extended Usage"),
            BoxCode::VIPR => Some("View priority Box"),
            BoxCode::VLAB => Some("WebVTT Source Label"),
            BoxCode::VMHD => {
                Some("video media header, overall information (video track only)")
            }
            BoxCode::VPBB => Some("Volumetric media bounding box"),
            BoxCode::VPCC => Some("VP9 Codec Configuration"),
            BoxCode::VSIB => Some("View scalability information"),
            BoxCode::VSSN => Some("viewing space"),
            BoxCode::VTTC => Some("WebVTTConfigurationBox"),
            BoxCode::VUNT => Some("V3C unit header"),
            BoxCode::VVCC => Some("VVC video configuration"),
            BoxCode::VVHD => Some("volumetric visual media header"),
            BoxCode::VVNC => Some("VVC Network Abstraction Layer unit configuration"),
            BoxCode::VWDI => Some("Multiview Scene Information"),
            BoxCode::VWID => Some("View identifier"),
            BoxCode::WIDE => Some("Expansion space reservation"),
            BoxCode::WMPI => Some("segment position in the watermark pattern"),
            BoxCode::XML => {
                Some("a tool by which vendors can add XML formatted information")
            }
            BoxCode::YRRC => Some("Year when media was recorded"),
            _ => None,
        }
    }
}
